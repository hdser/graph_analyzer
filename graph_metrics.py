    ...
    try:
        ...
        if len(lengths) > 1:
            ...
            if vals:
                ...
                result.update({k: 0 for k in ['avg_shortest_path', 'median_shortest_path', 
                              'max_shortest_path', 'path_variance', 'path_sum', 'reachable_nodes']})
...
def _compute_node_dominance(G, node, n):
    ...
    try:
        ...
        return node, {'dominated_nodes_count': dominated, 
...
class GraphMetrics:
    ...
    def __init__(self, graph: nx.DiGraph, n_jobs: int = None):
        ...
        logger.info(f"  • Is directed: True")
    ...
    def _distances(self, metrics):
        ...
        try:
            ...
            if nx.is_connected(self.U):
                ...
                logger.debug(f"  Radius: {radius}, Diameter: {diameter}")
                logger.debug(f"  Center nodes: {len(center)}, Periphery nodes: {len(periphery)}")
            else:
                logger.debug("  Graph disconnected, using largest component")
                largest_cc = max(nx.connected_components(self.U), key=len)
                subgraph = self.U.subgraph(largest_cc)
...
def load_data(user, password, host, database):
    """Load trust data from PostgreSQL"""
    logger.info("")
    logger.info("="*70)
